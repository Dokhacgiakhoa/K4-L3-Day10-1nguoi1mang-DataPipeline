...
| **4** | **Embedding & Vector Store Indexing** | **10** | `src/core/embeddings.py` (hoặc module vector), ChromaDB local persist directory | Khởi tạo thành công ChromaDB collection, sinh vector embedding từ mô hình `all-MiniLM-L6-v2`, index 24 tài liệu đầy đủ metadata truy vấn. |
| **5** | **Multi-Provider QA Agent** | **10** | `src/agent/` router và model clients | Hệ thống Agent hỗ trợ cơ chế chuyển đổi linh hoạt giữa các LLM provider (`mock`, `google`, `openai`, `anthropic`). Xử lý prompt chặt chẽ, trích xuất câu trả lời chuẩn xác từ context. |
